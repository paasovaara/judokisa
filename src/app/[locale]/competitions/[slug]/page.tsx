    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mb-6">
        <div className="mb-3 flex flex-wrap gap-2">
          ...
          <Badge
            label={tStatus(competition.status)}
        ...
        <div className="md:col-span-1">
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-5 space-y-4">
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {competition.categories.map((cat: string) => (
                    <span
                      key={cat}
                      className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs text-primary"
                    >
                      {cat}
                    </span>
                  ))}
            ...
            {competition.registrationDeadline && competition.status === "UPCOMING" && !isFull && (
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-gray-400">
                  {tComp("registration_deadline")}
                </p>
                <p className="mt-1 text-sm text-gray-700">
                  {fmt.format(competition.registrationDeadline)}
                </p>
              </div>
            )}
            {competition.registrationUrl && competition.status === "UPCOMING" && !isFull && (
              <a
                ...
                className="block w-full rounded-lg bg-primary px-4 py-2.5 text-center text-sm font-semibold text-white transition-opacity hover:opacity-90"
              >
                {tComp("register")} →
              </a>
            )}
            {isFull && (
              <span className="block rounded-lg bg-danger/10 px-4 py-2.5 text-center text-sm font-semibold text-danger">
                {tComp("full")}
              </span>
            )}
        ...
        <div className="space-y-8 md:col-span-2">
          ...
          {competition.videoUrl && (
            <section>
              ...
              {competition.videoUrl.includes("youtube.com") ||
              competition.videoUrl.includes("youtu.be") ? (
            ...
            {competition.status !== "COMPLETED" || competition.results.length === 0 ? (
              ...
              <div className="space-y-6">
                {(Object.entries(resultsByCategory) as [string, ResultRow[]][]).map(([category, results]) => (
                  <div key={category}>
                    <h3 className="mb-2 text-sm font-semibold text-primary">{category}</h3>
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 text-left text-xs font-medium uppercase tracking-wide text-gray-500">
                      <th className="py-2 pr-3">{locale === "fi" ? "Sarja" : "Category"}</th>
                      <th className="py-2 pr-3">{locale === "fi" ? "Kierros" : "Round"}</th>
                      <th className="py-2 pr-3">{locale === "fi" ? "Ottelu" : "Match"}</th>
                      <th className="py-2">{locale === "fi" ? "Tulos" : "Score"}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {competition.matches.map((m) => (
                      <tr key={m.id} className="border-b border-gray-100 last:border-0">
                        ...
                        <td className="py-2.5 pr-3">
                          <span
                            className={m.winnerName === m.athlete1Name ? "font-semibold text-gray-900" : "text-gray-600"}
                          >
                            {m.athlete1Name}
                          </span>
                          <span className="mx-2 text-gray-400">vs</span>
                          <span
                            className={m.winnerName === m.athlete2Name ? "font-semibold text-gray-900" : "text-gray-600"}
                          >
                            {m.athlete2Name}
                          </span>
                        </td>
